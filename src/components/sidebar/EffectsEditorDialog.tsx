...
export function EffectsEditorDialog(props: {
  ...
  useEffect(() => {
    if (!open) return;
    setConfig(normalizeEffectsConfig(loadEffectsConfig(room)) ?? DEFAULT_EFFECTS);
    setSelected(null);
    setTab('critical');
  }, [open, roomId]);
  ...
  const updatePc = (characterId: string, patch: Partial<NonNullable<EffectsConfig['pc']>[string][EffectKind]>) => {
    setConfig((prev) => {
      ...
      const currentEffect = (current as any)[tab] || { tag: '', x: 0, y: 0, scale: 1, rotate: 0, opacity: 1, z: 0 };
  ...
  const addPcPlaceholders = () => {
    setConfig((prev) => {
      ...
      pcCharacters.forEach((c, idx) => {
        ...
        (cur as any)[tab] = { tag: '', x, y, scale: 1, rotate: 0, opacity: 1, z: 100 };
        pc[c.id] = cur;
      });
      next.pc = pc;
      return next;
