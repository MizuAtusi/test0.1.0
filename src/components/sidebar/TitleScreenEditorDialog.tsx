...
export function TitleScreenEditorDialog(props: {
  ...
  const getImageOrder = (images: EffectImage[]) =>
    images
      .map((img, idx) => ({ img, idx }))
      .sort((a, b) => {
  ...
  const canMoveImage = (imageId: string, direction: 'front' | 'back') => {
    const ordered = getImageOrder(config.images || []);
  ...
  const moveImageLayer = (imageId: string, direction: 'front' | 'back') => {
    setConfig((prev) => {
      const next = normalizeTitleScreenConfig(prev);
      const ordered = getImageOrder(next.images || []);
      const idx = ordered.findIndex((img) => img.id === imageId);
      ...
      next.images = (next.images || []).map((img) => ({ ...img, z: zMap.get(img.id) ?? img.z }));
      return next;
    });
  };
