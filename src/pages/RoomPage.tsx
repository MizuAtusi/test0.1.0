...
const STAGE_HEIGHT_RATIO = 0.55; // height proportion of available stage area (stacked layout)
...
export default function RoomPage() {
  ...
  const stageHeightPxStacked = Math.max(0, availableStageHeightOverlay * STAGE_HEIGHT_RATIO);
  const stageHeightPxOverlay = availableStageHeightOverlay; // fill available space so it touches input bar
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ flexBasis: '75%' }}>
          {isStackedLayout ? (
            <div className="flex-1 min-h-0 px-4 pt-4 pb-0 flex flex-col gap-2">
              <div className="min-h-0" style={{ height: stageHeightPxStacked }}>
            ...
            <div className="flex-1 min-h-0 px-4 pt-2 pb-0 flex items-stretch">
              <div className="w-full" style={{ height: stageHeightPxOverlay }}>
                <StageFrame className="h-full w-full" ratio={16 / 9}>
                  <StageView
                    messages={messages.filter(m => m.channel !== 'chat')}
                    stageState={stageState}
